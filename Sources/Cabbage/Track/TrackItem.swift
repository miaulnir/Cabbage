...
public extension TrackItem {
    ...
    func generateFullRangeImageGenerator(size: CGSize = .zero, screenScale: CGFloat) -> AVAssetImageGenerator? {
        let item = makeFullRangeCopy()
        let imageGenerator = AVAssetImageGenerator.create(from: [item], renderSize: size)
        imageGenerator?.updateAspectFitSize(size, screenScale: screenScale)
